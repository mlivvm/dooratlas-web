...
function getSelectSheetItems(type, context = {}) {
    if (type === 'customer') {
        return FD.SelectSheetService.buildCatalogCustomerItems(customers);
    }
    ...
    if (type === 'location')
        return [];
    return FD.SelectSheetService.buildCatalogFloorplanItems(customer, {
        groupByLocation: !topbarFloorplanLocationFilter,
        descriptionForItem: fp => floorplanAddressMeta(customer, fp),
        metaForItem: fp => floorplanPermissionMeta(customer, fp),
        readOnlyForItem: fp => isViewerReadOnlyFloorplan(customer.customer, fp.name),
    });
}
