(function (global) {
    ...
    function getConfiguredSvgSize(svgEl) {
        if (!svgEl)
            throw new Error('Geen geldig SVG bestand.');
        const vb = svgEl.viewBox?.baseVal;
        if (!vb?.width || !vb?.height)
            throw new Error('SVG heeft geen geldige viewBox.');
    ...
    function createLoadController({ elements, getSelection, fetchSvg, setLoadingState, onBeforeLoad, onSvgReady, onBeforeReveal, onRevalidated, canApplyRevalidated, onError, loadingDelayMs = 150, }) {
        let generation = 0;
        ...
        async function renderSvg(svgText, context, runGeneration, selection, { revalidated = false, revalidation } = {}) {
            const { svgContainer, loadingEl } = elements;
            if (!isCurrent(runGeneration, selection))
                return false;
            if (revalidated) {
            ...
            if (typeof onSvgReady === 'function') {
                await onSvgReady({ svgEl, size, context, revalidated, revalidation });
            }
            if (!revalidated)
                await nextFrame();
            if (!isCurrent(runGeneration, selection))
            ...
            if (typeof onBeforeReveal === 'function') {
                await onBeforeReveal({ svgEl, size, context, revalidated, revalidation });
            }
        ...
        async function load(context) {
            ...
            try {
                ...
                if (revalidate) {
                    revalidate.then(async (result) => {
                        if (!result?.changed || !result.svgText || !isCurrent(runGeneration, selection) ||
                            (typeof canApplyRevalidated === 'function' && !canApplyRevalidated()))
                            return;
                        try {
                            const rendered = await renderSvg(result.svgText, context, runGeneration, selection, { revalidated: true, revalidation: result });
                            if (rendered && typeof onRevalidated === 'function')
                                onRevalidated(context);
                        }
                        catch {
                            elements.svgContainer.style.visibility = '';
                        }
                ...
                if (activeController === requestController)
                    activeController = null;
                showError(err instanceof Error ? err : new Error(String(err)), runGeneration, selection);
            }
            finally {
