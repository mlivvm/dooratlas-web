(function (global) {
    ...
    function createLoadController({ elements, getSelection, fetchSvg, setLoadingState, onBeforeLoad, onSvgReady, onBeforeReveal, onRevalidated, onError, loadingDelayMs = 150, }) {
        let generation = 0;
            ...
            try {
                ...
                if (revalidate) {
                    revalidate.then(async (newSvgText) => {
                        if (!newSvgText || !isCurrent(runGeneration, selection))
                            return;
                        try {
