(function (global) {
    ...
    function resetPdfState(elements) {
        ...
        // These source chips sit outside the PDF panes, so reset them explicitly too.
        if (elements.sourceList)
            elements.sourceList.innerHTML = '';
        if (elements.pdfSourceList)
            elements.pdfSourceList.innerHTML = '';
    ...
    async function showPdfPagePreview(elements, page) {
        ...
        const loadPreview = !page.editDataUrl && !page.previewDataUrl && pdfDocument && page.sourceType === 'pdf'
            ? async () => {
            ...
            : null;
        await FD.UploadPdfPreview.open({
            title: `${Pdf.pdfPageLabel(page)} bekijken`,
            alt: `${Pdf.pdfPageLabel(page)} vergroot`,
            immediatePreview,
            loadPreview,
        });
    }
