(function (global) {
    ...
    function markerFitsInsideCrop(marker, cropX, cropY, cropW, cropH) {
        const bounds = FD.MarkerService?.markerBounds(marker);
        if (!bounds)
            return false;
        return bounds.left >= cropX && bounds.right <= cropX + cropW &&
            bounds.top >= cropY && bounds.bottom <= cropY + cropH;
    }
    ...
    function buildCroppedSVGText({ svgEl, imageDataUrl, plan, markerService = FD.MarkerService, serializer = new XMLSerializer(), }) {
        ...
        svgClone.querySelectorAll('[data-door-id]').forEach(marker => {
            ...
            if (!position)
                return;
            const bounds = markerService.markerBounds(marker);
            if (bounds.left < plan.cropX || bounds.right > plan.cropX + plan.cropW || bounds.top < plan.cropY || bounds.bottom > plan.cropY + plan.cropH) {
                marker.remove();
                return;
            }
