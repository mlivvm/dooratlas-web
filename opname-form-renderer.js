(function (global) {
    ...
    function open(options, helpers) {
        ...
        function persistDraft(force = false) {
            syncVisibleInputs();
            if (force || helpers.hasChanges(helpers.collectPayload('opname', state), baseline))
                return options.onDraft?.(structuredClone(state));
            if (!startingDraft)
                return options.onDraft?.(null);
            return undefined;
        }
        function queueDraft() {
            if (!options.onDraft)
        ...
        const contentRenderer = Content.create({
            ...
            persistDraft, rerender: (errorText = '') => render(errorText), renderSummary,
        });
