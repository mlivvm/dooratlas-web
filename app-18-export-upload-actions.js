...
function handleEditTapOnEmpty(e) {
    ...
    if (!svgPoint || !isPointInsideEditableBounds(svgPoint.x, svgPoint.y))
        return;
    if (activeMarkerShape === 'polygon') {
        addPolygonDraftPoint(svgPoint.x, svgPoint.y);
        return;
    }
    if (beginNewShapePlacement(svgPoint.x, svgPoint.y))
        return;
    if (autoNumbering) {
    ...
    showDoorMetadataPopup({
        title: 'Nieuwe deur', submitText: 'Toevoegen', lockOverlayDismiss: true,
        onSubmit: ({ doorCode, description, securityLevel }) => {
            ...
            addMarkerAtPosition(svgPoint.x, svgPoint.y, doorCode, description, securityLevel);
            closeEditPopup();
        },
    });
    if (previewMarker)
