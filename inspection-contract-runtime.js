(() => {
  ...
  function validationErrors(contract2, formType, step, state, context = {}) {
    ...
    const errors = [...required].filter((name) => !hasValue(state.fields[name])).map((name) => `${findField(contract2, formType, name)?.label ?? name} is verplicht.`);
    if (step.matrixKey) {
      const matrix = findMatrix(contract2, formType, step.matrixKey);
      const statusColumns = matrix?.statusColumns ?? [];
      const actionColumns = (matrix?.columns ?? []).filter((column) => !statusColumns.includes(column));
      (step.rows ?? []).forEach((row) => {
        if (!state.matrices[matrixKey(step.matrixKey, row, "Nee")]) return;
        const hasAction = actionColumns.some((column) => state.matrices[matrixKey(step.matrixKey, row, column)]);
        if (!hasAction) errors.push("Kies minstens \xE9\xE9n actie.");
      });
    }
    return errors;
  }
