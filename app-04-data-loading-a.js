...
function doorDetailsForMarker(marker, doorId = '') {
    ...
    const opnamePresence = marker?.getAttribute?.('data-fd-has-opname');
    const onderhoudPresence = marker?.getAttribute?.('data-fd-has-onderhoud');
    const hasOpname = opnamePresence === null ? null : opnamePresence === '1';
    const hasMaintenance = onderhoudPresence === null ? null : onderhoudPresence === '1';
    const metaParts = [];
    if (name && name !== label)
        metaParts.push(name);
    ...
    return {
        ...
        hasOpname,
        hasMaintenance,
        hasAnyInspection: hasOpname === null || hasMaintenance === null ? null : hasOpname || hasMaintenance,
        meta: metaParts.join(' · '),
    };
}
