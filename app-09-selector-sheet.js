...
function adminFloorplanSearchText(record) {
    return [
        ...
        record.displayName,
        record.name,
        FD.SelectSheetService.floorplanLocationFilterLabel(FD.SelectSheetService.floorplanLocationFilterValue(record)),
        record.building,
        record.floorLabel,
...
function getSelectedAdminDoor() {
    if (!adminDashboardState.selectedDoorKey)
        return null;
    const door = (getAdminData().doors || []).find(item => adminDoorKey(item) === adminDashboardState.selectedDoorKey);
    return door || adminDashboardState.activity.find(item => adminDoorKey(item) === adminDashboardState.selectedDoorKey) || null;
}
function setAdminTab(tabName) {
    const allowed = new Set(['overview', 'conflicts', 'door-search', 'floorplans', 'details']);
...
function getAdminCustomerOptions() {
    const records = [
        ...(getAdminData().customers || []),
        ...(getAdminData().floorplans || []),
        ...(customers || []),
    ];
...
function getFilteredAdminFloorplans() {
    ...
    return (Array.isArray(data.floorplans) ? data.floorplans : []).filter(record => {
        if (selectedCustomer && adminTenantKey(record) !== selectedCustomer)
            return false;
        if (selectedLocation && FD.SelectSheetService.floorplanLocationFilterValue(record) !== selectedLocation)
            return false;
