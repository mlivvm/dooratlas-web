(function (global) {
    ...
    function create(deps) {
        const { Flow, state, options, helpers, element, readFile, getFieldError, fieldRequired, setField, persistDraft, rerender, renderSummary, Ui } = deps;
        ...
        function renderPhoto(kind) {
            ...
            const addNativePhoto = async () => {
                ...
                try {
                    ...
                    const photo = await adapter.capture({ allowGallery: kind === 'upload_hier_alle_vooraf_genomen_foto_s', photoKind: kind });
                    if (!photo)
                        return;
                    const current = state.photos[kind] || [];
                    const maximum = Math.max(1, Number(item.maxFiles || 1));
