function canManageUploads(target = currentFloorplanPermissionTarget()) {
    return FD.DataService.canManageUploads(CONFIG, target);
}
function canStartAuthoring() {
    return FD.DataService.canStartAuthoring(CONFIG);
}
...
function canUseExcelExport() {
    return ['admin', 'sales', 'viewer'].includes(currentUser?.role || '');
}
...
function metadataSaveErrorText(err, duplicateMessage) {
    if (err?.code === LOCATION_DETAILS_SAVE_ERROR)
        return err.message;
    if (err?.status === 409 && /intussen gewijzigd|verouderd/i.test(String(err?.message || ''))) {
        return 'De gegevens zijn intussen gewijzigd. Sluit dit venster, open het opnieuw en probeer nogmaals.';
    }
    if (err?.status === 409)
        return duplicateMessage;
    return 'Opslaan mislukt: ' + (err?.message || 'onbekende fout');
}
function applyMetadataConflictCustomers(nextCustomers) {
    if (!Array.isArray(nextCustomers) || !nextCustomers.length)
        return;
    const selected = getSelectedFloorplan();
    const priorCustomer = selected.customer;
    const priorFloorplan = selected.floorplan;
    customers = nextCustomers;
    cacheCustomers();
    populateCustomerDropdown();
    const customerIndex = customers.findIndex(customer => Number(customer?.tenantId) === Number(priorCustomer?.tenantId));
    if (customerIndex < 0)
        return;
    customerSelect.value = String(customerIndex);
    populateFloorplanDropdown(customerIndex);
    const floorplanIndex = (customers[customerIndex].floorplans || []).findIndex(item => Number(item?.floorId) === Number(priorFloorplan?.floorId));
    if (floorplanIndex >= 0)
...
function getSelectedTopbarFloorplanRecord() {
    ...
    return {
        ...
        tenantId: floorplan.tenantId || customer.tenantId || 0,
        name: floorplan.name,
        ...
        locationRevision: floorplan.locationRevision || '',
...
function updateTopbarMetadataButton() {
    ...
    const visible = canManageUploads() && FD.DataService.isFloorplanMetadataWriteEnabled?.(CONFIG);
...
function canUseTopbarFloorplanActions() {
    const { customer, floorplan } = getSelectedFloorplan();
    if (!customer || !floorplan || adminDashboardState.visible || !appMode.isInteractiveView() || !hasCurrentFloorplanView())
        return false;
