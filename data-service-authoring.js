(function (global) {
    ...
    async function saveFloorAuthoring(config, floorId, svgText, doorUpdates, newDoors, deletedDoorIds, options = {}) {
        return C.requestJson(config, `/api/floors/${floorId}/authoring-save`, {
            ...
            body: {
                ...
                door_updates: doorUpdates.map(update => ({ door_id: update.doorId, door_code: update.doorCode, name: update.name, security_level: update.securityLevel })),
                new_doors: newDoors.map(door => ({ draft_key: door.draftKey, door_code: door.doorCode, name: door.name, security_level: door.securityLevel })),
    ...
    function prepareSvgForDoorSave(floorplan, svgText, securityLevels = {}) {
        const doc = parseSvgDocument(svgText);
        const sourceRevision = String(doc.documentElement.getAttribute('data-fd-source-revision') || '');
        doc.documentElement.removeAttribute('data-fd-source-revision');
        ...
        for (const marker of markers) {
            ...
            if (existing) {
                ...
                const markerKey = F.markerKey(existing);
                const hasSecurityLevel = Object.prototype.hasOwnProperty.call(securityLevels, markerKey);
                const desiredSecurityLevel = hasSecurityLevel
                    ? (F.normalizeName(securityLevels[markerKey]).toLowerCase() || null)
                ...
                const changed = metadataEdited && Boolean(desiredCode) && (desiredCode !== F.normalizeName(existing.door_code) ||
                    desiredDescription !== doorDescription(existing) ||
                    desiredSecurityLevel !== existingSecurityLevel);
                if (changed)
                    doorUpdates.push({
                        doorId: Number(existing.id),
                        doorCode: desiredCode,
                        name: desiredDescription || null,
                        securityLevel: desiredSecurityLevel,
                    });
                applyDoorMetadataToMarker(marker, { ...existing, door_code: desiredCode, name: desiredDescription || desiredCode, security_level: desiredSecurityLevel }, { preserveTechnicalAnchor: !hasTechnicalAnchor(existing) });
            ...
            newDoors.push({ draftKey, doorCode: manualCode, name: manualDescription || null, securityLevel });
            marker.removeAttribute(DOOR_METADATA_EDITED_ATTR);
        }
    ...
    async function saveFloorplanSVG(fileUrl, svgText, options = {}) {
        ...
        try {
            const prepared = prepareSvgForDoorSave(floorplan, svgText, options.securityLevels);
            const expectedRevision = String(prepared.sourceRevision || '');
    ...
    async function addUploadedFloorplan(config, payload, options = {}) {
        ...
        C.floorplanCache.delete(Number(floor.id));
        const customers = await W.loadCustomers(config);
        return { customers, floorplan: floorplanFromMutation(floor, tenant) };
    }
    FD.DataServiceAuthoring = {
        addUploadedFloorplan,
        applyDoorMetadataToMarker,
        assertNoCurrentFloorDuplicateCodes,
        createLocation,
        createTenant,
        doorByMarker,
        doorDescription,
        expectedLevelOrder: F.expectedLevelOrder,
        hasTechnicalAnchor,
        createInspection,
        listLocations,
        markerDescription,
        markerManualCode,
        markerMetadataWasEdited,
        saveFloorplanSVG,
        suggestLevelOrder: F.suggestLevelOrder,
        levelOrderWarning: F.levelOrderWarning,
    };
})(window);
