...
function applyMapMode(nextMode, { persist = false } = {}) {
    const previousMode = mapMode;
    mapMode = FD.MapModeService.normalizeMapMode(nextMode);
    ...
    if (!persist)
        return;
    FD.DataService.saveMapModePreference(CONFIG, mapMode)
        ...
        if (!FD.DataService.isSessionAuthError?.(err)) {
            showToast('Weergavevoorkeur kon niet worden opgeslagen.', 'error');
        }
    });
}
function syncMapModeFromCurrentUser() {
    applyMapMode(currentUser?.lastMapMode || 'opname');
}
mapModeController = FD.MapModeService.createController({
    rootEl: mapModeControl,
    buttonEl: btnMapMode,
    labelEl: mapModeLabel,
    menuEl: mapModeMenu,
    optionEls: mapModeOptions,
    onChange: nextMode => applyMapMode(nextMode, { persist: true }),
});
...
let topbarHeightFrame = null;
function settleTopbarHeight() {
    if (topbarHeightFrame !== null)
        return;
    topbarHeightFrame = window.requestAnimationFrame(() => { topbarHeightFrame = null; updateTopbarHeight(); });
}
...
if (typeof ResizeObserver === 'function') {
    new ResizeObserver(updateTopbarHeight).observe(topbarEl);
}
updateViewportMetrics();
updateTopbarHeight();
settleTopbarHeight();
...
async function remoteDeploymentReady(remote) {
    ...
    try {
        ...
        return indexHtmlReady(textByAsset.get('index.html'), remote.version) &&
            ...
            textByAsset.get('office.bundle.js')?.includes(`version=${remote.version};`) &&
