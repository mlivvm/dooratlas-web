    ...
    function createAuthController({ loginConfig, appConfig, elements, logoutControls, modeController, modes, emailConfig = {}, emailjsClient = global.emailjs, hideTopbarMenu = () => { }, showToast = () => { }, onShowApp = () => { }, onLogout = () => { }, onSessionExpired = () => { }, logger = console, }) {
        ...
        async function getEmailClient() {
            if (emailConfig.enabled === false)
                return null;
            const client = emailjsClient || await FD.RuntimeLibraryLoader?.ensureEmailJs?.();
            if (!emailInitialized && emailConfig.publicKey && client?.init) {
                client.init(emailConfig.publicKey);
                emailInitialized = true;
            }
            return client;
        }
        async function notifyLogin(type, attempts) {
            if (emailConfig.enabled === false)
                return;
            try {
                N.sendLoginNotification({
                    emailjsClient: await getEmailClient(),
                ...
                logger.warn?.('Loginmelding kon niet worden geladen:', err);
            }
        }
        function hideSplash() {
        ...
        async function ensureWorkerSessionForStoredLogin() {
            ...
            if (global.navigator?.onLine === false && hasValidWorkerSession())
                return true;
            try {
                // A page reload deliberately has no persisted CSRF token. One session/me
                // validates revocation and restores the current token only in memory.
        ...
        function showLoginDuringLogout() {
            hideSplash();
            elements.appContainer.style.display = 'none';
            elements.loginScreen.style.display = 'flex';
            elements.errorEl.textContent = 'Bezig met uitloggen...';
            setLoginEnabled(false);
        }
        ...
        function setLogoutBusy(busy) {
            logoutBusy = busy;
            logoutControls.popup?.setAttribute('aria-busy', busy ? 'true' : 'false');
            logoutControls.confirmButton.disabled = busy;
            logoutControls.cancelButton.disabled = busy;
            logoutControls.confirmButton.textContent = busy ? 'Uitloggen…' : 'Uitloggen';
        }
