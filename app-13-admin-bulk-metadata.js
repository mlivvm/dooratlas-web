...
function renderAdminDashboard() {
    ...
    renderAdminKpis();
    if (adminDashboardState.activeTab === 'overview') {
        renderAdminOverview();
        renderAdminActivity();
    }
    else if (adminDashboardState.activeTab === 'door-search') {
        renderAdminDoorResults();
        void ensureAdminDoorIndex();
    }
    else if (adminDashboardState.activeTab === 'conflicts') {
        renderAdminConflicts();
    }
    else if (adminDashboardState.activeTab === 'floorplans') {
...
async function ensureAdminDoorIndex() {
    const data = adminDashboardState.data;
    if (!data || adminDashboardState.doorIndexLoaded || adminDashboardState.doorIndexLoading)
        return;
    adminDashboardState.doorIndexLoading = true;
    adminDashboardState.doorIndexError = '';
    renderAdminDoorResults();
    const generation = FD.DataService.sessionGeneration?.();
    try {
        const result = await FD.DataService.fetchAdminDoorIndex(CONFIG, {
            diagnostics: { purpose: 'admin_door_index', background: true },
        });
        if (generation !== FD.DataService.sessionGeneration?.() || adminDashboardState.data !== data)
            return;
        data.doors = Array.isArray(result.doors) ? result.doors : [];
        adminDashboardState.doorIndexLoaded = true;
    }
    catch (err) {
        if (generation !== FD.DataService.sessionGeneration?.() || adminDashboardState.data !== data)
            return;
        console.warn('Admin deurcodes laden mislukt:', err);
        adminDashboardState.doorIndexError = err.message || 'door_index_failed';
    }
    finally {
        if (adminDashboardState.data === data) {
            adminDashboardState.doorIndexLoading = false;
            if (adminDashboardState.activeTab === 'door-search')
                renderAdminDoorResults();
        }
    }
}
async function loadAdminDashboard({ force = false } = {}) {
    ...
    try {
        ...
        adminDashboardState.doorIndexLoaded = false;
        adminDashboardState.doorIndexLoading = false;
        adminDashboardState.doorIndexError = '';
...
function invalidateAdminDashboard() {
...
function hideAdminDashboard() {
    ...
    stopAdminConflictPolling();
    resetAdminPreview('Preview laden na selectie');
...
async function loadAdminPreview(record) {
    ...
    adminDashboardState.previewController = controller;
    adminDetailPreview.textContent = 'Preview laden...';
    try {
        const result = await fetchFloorplanSVGCacheFirst(getFloorplanApiUrl(record), { signal: controller.signal });
        ...
        if (adminDashboardState.previewController === controller)
            adminDashboardState.previewController = null;
    }
}
function cancelAdminPreviewRequest() {
    adminDashboardState.previewController?.abort();
    adminDashboardState.previewController = null;
}
