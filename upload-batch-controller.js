(function (global) {
    ...
    function attachBatchHandlers(pdfHandlers, ctx) {
        const { elements, controls, modeController, modes, ensureSession, onSave, onSaved, showToast, hidePopup, currentCustomers, materializeContext = async (context) => context, rememberSuccessfulCustomer = () => { }, validationCustomers = currentCustomers, } = ctx;
        function showPdfFormForCurrentCustomers() {
            ...
            elements.pdfNamesList?.scrollTo({ left: 0, top: 0 });
        }
        async function savePdfBatchUpload() {
            ...
            let form = validateCurrentPdfBatch(pagesToUpload, validationCustomers());
            if (!form.ok) {
            ...
            try {
                await materializeContext(elements.pdfState.uploadContext);
                form = validateCurrentPdfBatch(pagesToUpload, currentCustomers());
                if (!form.ok) {
                    elements.pdfErrorEl.textContent = form.error;
                    return;
                }
                ...
                elements.pdfErrorEl.textContent = `Klant of locatie kon niet worden opgeslagen. ${Core.formatUploadError(err)}`;
                return;
            }
            await uploadValidatedPdfBatch(form);
        }
        function validateCurrentPdfBatch(pagesToUpload, customers = currentCustomers()) {
        ...
        function finishPdfBatchUpload(form, result) {
            ...
            rememberSuccessfulCustomer(form.tenantId);
            hidePopup();
