(function (global) {
    ...
    function createSelectionController(options) {
        const { elements, onFilterChange, onCustomerChange, onFloorplanChange, onPickerOpen } = options;
        let bound = false;
        const continueCustomerSelectionOnTouch = () => Boolean(global.matchMedia?.('(hover: none) and (pointer: coarse)').matches);
        const sheetController = D.createController({
            ...
            onSelect: (type, item) => {
                if (type === 'customer') {
                    ...
                    if (continueCustomerSelectionOnTouch())
                        open('floorplan');
                }
        ...
        function open(type) {
            if (typeof onPickerOpen === 'function') {
                onPickerOpen({ type, customerIndex: selectedIndex(elements.customerSelect) });
            }
            sheetController.open(type);
        }
        function bind() {
            if (bound)
                return;
            bound = true;
            elements.desktopContextPickerBtn?.addEventListener('click', () => open('customer'));
            elements.customerPickerBtn.addEventListener('click', () => open('customer'));
            elements.floorplanPickerBtn.addEventListener('click', () => open('floorplan'));
    ...
    FD.SelectSheetService = {
        ...
        buildCatalogCustomerItems: S.buildCatalogCustomerItems,
        buildCatalogFloorplanItems: S.buildCatalogFloorplanItems,
