(function (global) {
    ...
    function locationDescription(customer, floorplan) {
        const details = getFloorplanLocationDetails(customer, floorplan);
        if (!details)
            return '';
        return [
            String(details.address || '').trim(),
            details.note ? `Notitie: ${String(details.note).trim()}` : '',
        ].filter(Boolean).join(' · ');
    }
    function organizer(floorplan) {
        const location = floorplanLocationName(floorplan);
        return location
            ? { key: `location:${location}`, label: location, kind: 'location', none: false }
            : { key: ORGANIZER_NONE_VALUE, label: ORGANIZER_NONE_LABEL, kind: 'none', none: true };
    }
    function compareCatalogFloorplans(left, right) {
        if ((left.organizerNone ? 1 : 0) !== (right.organizerNone ? 1 : 0)) {
            return left.organizerNone ? 1 : -1;
        }
        const organizerCompare = LABEL_COLLATOR.compare(left.organizerLabel || '', right.organizerLabel || '');
        if (organizerCompare)
            return organizerCompare;
        const displayCompare = compareFloorplanDisplayOrder(left.floorplan, right.floorplan, left.label, right.label);
        if (displayCompare)
            return displayCompare;
        const labelCompare = LABEL_COLLATOR.compare(left.label, right.label);
        return labelCompare || left.index - right.index;
    }
    function buildCatalogCustomerItems(customers) {
        return sortedWithOriginalIndex(customers, customer => customer?.customer)
            .map(({ item, index, label }) => {
            const count = Array.isArray(item?.floorplans) ? item.floorplans.length : 0;
            return { index, label, meta: `${count} plattegrond${count === 1 ? '' : 'en'}`, customer: item };
        });
    }
    function buildCatalogFloorplanItems(customer, options = {}) {
        const floorplans = Array.isArray(customer?.floorplans) ? customer.floorplans : [];
        const hasOrganizerBlocks = floorplans.some(floorplan => floorplanLocationName(floorplan));
        const groupByLocation = options.groupByLocation !== false;
        const descriptionForItem = typeof options.descriptionForItem === 'function'
            ? options.descriptionForItem
            : (floorplan) => locationDescription(customer || {}, floorplan);
        const metaForItem = typeof options.metaForItem === 'function'
            ? options.metaForItem
            : () => '';
        const readOnlyForItem = typeof options.readOnlyForItem === 'function'
            ? options.readOnlyForItem
            : () => false;
        const localStateForItem = typeof options.localStateForItem === 'function'
            ? options.localStateForItem
            : () => null;
        return sortedWithOriginalIndex(floorplans, floorplan => floorplanDisplayName(floorplan))
            .map(({ item: floorplan, index, label }) => {
            const locationValue = floorplanLocationFilterValue(floorplan);
            const organizerMeta = organizer(floorplan);
            const description = String(descriptionForItem(floorplan, customer) || '').trim();
            const meta = String(metaForItem(floorplan, customer) || '').trim();
            return {
                index,
                label,
                meta,
                description,
                filterValues: { location: locationValue },
                floorplan,
                locationValue,
                organizerKey: organizerMeta.key,
                organizerLabel: organizerMeta.label,
                organizerKind: organizerMeta.kind,
                organizerNone: organizerMeta.none,
                collapsibleGroupKey: hasOrganizerBlocks ? organizerMeta.key : '',
                collapsibleGroupLabel: hasOrganizerBlocks ? organizerMeta.label : '',
                groupLabel: groupByLocation
                    ? floorplanLocationFilterLabel(locationValue)
                    : organizerMeta.label,
                searchText: [
                    label,
                    description,
                ...
                localState: localStateForItem(floorplan, customer),
                readOnly: Boolean(readOnlyForItem(floorplan, customer)),
            };
        })
            .sort(compareCatalogFloorplans);
    }
    FD.SelectSheetCore = {
        LOCATION_ALL_VALUE,
        LOCATION_NONE_VALUE,
        ...
        buildCatalogCustomerItems,
        buildCatalogFloorplanItems,
