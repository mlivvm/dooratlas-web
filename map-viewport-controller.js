...
(function (global) {
    ...
    function create(input) {
        ...
        function refreshMetrics() {
            ...
            const contentBounds = svgEl ? input.getContentBounds?.(svgEl) : null;
            const content = contentBounds || (viewBox ? {
                x: viewBox.x || 0, y: viewBox.y || 0, width: viewBox.width, height: viewBox.height,
            } : null);
            if (!svgEl || !content?.width || !content.height) {
                metrics = null;
                return null;
            }
        ...
        function clientToSvgPoint(clientX, clientY) {
            ...
            return viewport.clientToSvgPoint({
                ...
                panY: current.panY,
                scale: current.scale,
                viewBoxX: currentMetrics.contentX,
                viewBoxY: currentMetrics.contentY,
            });
        }
        return {
            refreshMetrics,
