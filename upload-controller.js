(function (global) {
    ...
    function createUploadController({ elements, controls, getCustomers, refreshCustomers = async () => { }, getCurrentUser = () => null, modeController, modes, isEditMode = () => false, hideTopbarMenu = () => { }, showToast = () => { }, getPdfJsLib = () => global.pdfjsLib, ensurePdfLibrary = async () => getPdfJsLib(), ensureCropperLibrary = async () => global.Cropper, ensureSession = async () => true, onCreateTenant, onListLocations, onCreateLocation, onSave, onSaved = () => { }, }) {
        let generation = 0;
        let saving = false;
        let bound = false;
        let dragDepth = 0;
        ...
        function resetAll() {
            // Also clear the native picker so a finished upload always starts fresh.
        ...
        function showPopup() {
            ...
            wizard.setCustomerLoading(true);
            void Promise.resolve(refreshCustomers()).then(() => {
                if (refreshGeneration !== generation || !modeController.is(modes.UPLOAD))
                    return;
                wizard.reset({ allowNewCustomer: canCreateCustomer() });
                wizard.setCustomerLoading(false);
            }).catch(() => {
                if (refreshGeneration !== generation || !modeController.is(modes.UPLOAD))
                    return;
                wizard.setCustomerLoading(false);
                showToast('Klanten konden niet worden bijgewerkt. Controleer uw verbinding en probeer opnieuw.', 'error');
            });
        }
        ...
        const ctx = {
            controls, currentCustomers, elements, ensureSession, getPdfJsLib, ensureCropperLibrary,
            ...
            showToast,
            materializeContext: wizard.materializeContext,
            rememberSuccessfulCustomer: wizard.rememberSuccessfulCustomer,
            validationCustomers: wizard.validationCustomers,
        };
        ...
        async function handleFiles(fileList) {
            ...
            elements.uploadContext = context;
            elements.pdfState.uploadContext = context;
            const includesPdf = Array.from(fileList || []).some(file => file.type === 'application/pdf' || /\.pdf$/i.test(file.name));
            if (includesPdf) {
                try {
                    await ensurePdfLibrary();
                }
                catch {
                    showToast('PDF-tools konden niet worden geladen. Controleer de verbinding.', 'error');
                    return;
                }
            }
