(function (global) {
    ...
    function open(options, helpers) {
        ...
        const startingDraft = Boolean(options.draftState);
        ...
        function persistDraft(force = false) { if (force || helpers.hasChanges(Flow.collectPayload(state), baseline))
            return options.onDraft?.(structuredClone(state)); if (!startingDraft)
            return options.onDraft?.(null); return undefined; }
        function queueDraft() {
            if (!options.onDraft)
                return;
            if (draftTimer !== null)
        ...
        if (options.onDraft)
            ['input', 'change'].forEach(event => modal.addEventListener(event, queueDraft));
        ...
        function selectOption(name, value) {
            if (name === 'nul_beurt')
                ...
                if (name === 'controle_meerwerk_gedaan' && state.fields[name] !== 'Ja')
                    delete state.fields.controle_tijd_besteed_meerwerk;
            }
        ...
        const matrixError = (row) => renderedErrors.includes('Kies minstens één actie.') && Flow.matrixNeedsAction(state, steps().find(step => step.id === state.stepId)?.matrixKey || '', row) ? 'Kies minstens één actie.' : '';
        const contentRenderer = Content.create({ Flow, state, options, helpers, element, readFile, renderInputField, renderNulBeurt, renderSummary, matrixError, persistDraft, rerender: (errorText = '') => render(errorText) });
