async function saveAdminDetail() {
    ...
    try {
        const result = await FD.DataService.updateFloorplanRecord(CONFIG, {
            ...
            expectedFloorRevision: record.revision,
            expectedLocationRevision: record.locationRevision,
        ...
        applyMetadataConflictCustomers(err?.customers);
        if (adminDetailError) {
...
const selectionController = FD.SelectSheetService.createSelectionController({
    ...
    getItemKey: (type, item) => {
        if (type === 'customer')
            return `tenant:${Number(item?.customer?.tenantId || 0)}`;
        return `floor:${Number(item?.floorplan?.floorId || item?.floorplan?.file || 0)}`;
    },
    onFilterChange: handleSelectSheetFilterChange,
    onPickerOpen: ({ type, customerIndex }) => {
        if (!selectionController.isOpen())
            void refreshPickerCatalog();
        if (type === 'floorplan')
            void refreshFloorplanSummariesForCustomer(customerIndex);
    },
    onCustomerFocus: ({ customerIndex }) => {
        void refreshFloorplanSummariesForCustomer(customerIndex);
    },
...
const sidePanelController = FD.SidePanelService.createController({
    ...
    getDoorPresentation: doorId => {
        const marker = FD.MarkerService.findMarkerByDoorId(svgContainer, doorId);
        return marker ? FD.MapModeService.markerPresentation(marker, mapMode, COLORS) : null;
    },
