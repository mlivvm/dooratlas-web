...
function applyResize() {
    if (!resizingMarker)
        return;
    editChanges.push(FD.MarkerService.resizeChange(resizingMarker.doorId, resizingOldRx, resizingOldGeometry));
    clearResizeHighlight(resizingMarker.marker);
    resizingMarker = null;
    resizingOldRx = null;
    resizingOldGeometry = null;
...
function cancelResize() {
    if (!resizingMarker)
        return;
    if (resizingOldGeometry)
        FD.MarkerShapeService.setGeometry(resizingMarker.marker, resizingOldGeometry);
    else
...
function updateEditLabels() {
    ...
    const labels = FD.MarkerService.labelPlacements(svgContainer.querySelectorAll('[data-door-id]'), {
        scale,
        activeDoorId,
        bounds: mapCamera.contentBounds(svgEl),
    });
