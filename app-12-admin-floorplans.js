const ADMIN_DOOR_RESULT_LIMIT = 200;
...
function renderAdminDoorResults() {
    ...
    if (adminDashboardState.doorIndexLoading || adminDashboardState.doorIndexError) {
        const empty = document.createElement('div');
        empty.className = 'admin-dashboard-empty';
        empty.textContent = adminDashboardState.doorIndexLoading ? 'Deurcodes laden...' : 'Deurcodes laden mislukt.';
    ...
    results.sort(compareAdminDoors);
    const visibleResults = results.slice(0, ADMIN_DOOR_RESULT_LIMIT);
    ...
    const scope = adminDashboardState.doorFloorplanFilter
        ...
        : (adminDashboardState.doorCustomerFilter
            ...
            : 'in alle klanten');
    const limitNotice = results.length > visibleResults.length ? ' · verfijn de zoekopdracht voor de overige resultaten' : '';
    summary.textContent = `${results.length} deurcode${results.length === 1 ? '' : 's'} ${scope}${limitNotice}`;
...
function renderAdminCustomerSelect(record) {
    if (!adminDetailCustomer)
        return;
    adminDetailCustomer.value = adminCustomerLabel(record);
}
...
function openAdminMetadataDialog(record) {
    if (!record || !canManageUploads({ tenantId: record.tenantId }))
        return;
