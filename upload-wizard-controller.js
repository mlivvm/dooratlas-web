(function (global) {
    ...
    function createUploadWizard({ elements, controls, currentCustomers, getCurrentUser, onCreateTenant, onListLocations, onCreateLocation, onContinueFiles = () => { }, }) {
        ...
        const recentCustomer = FD.UploadRecentCustomer.createRecentCustomerShortcut({
            button: elements.recentCustomerButton,
            customerById,
        });
        ...
        function showStep(step) {
            ...
            recentCustomer.sync(activeStep, creationMode);
        ...
        async function reuseRecentCustomer() {
            const customer = recentCustomer.customer();
            if (!customer)
                return;
            const tenantId = Number(customer.tenantId || customer.id || 0);
            if (!tenantId)
                return;
            setError();
            customerCombobox.selectValue(String(tenantId), { emit: false });
            await handleCustomerChange();
            if (Number(elements.customerSelect.value || 0) === tenantId)
                showStep('location');
        }
        ...
        function bind() {
            ...
            elements.recentCustomerButton?.addEventListener('click', () => { void reuseRecentCustomer(); });
        ...
        return { bind, currentContext, materializeContext, refreshFileState, rememberSuccessfulCustomer: recentCustomer.remember, reset, setCustomerLoading, showFileStep, showStep, validationCustomers };
    }
