(function (global) {
    ...
    function matrixNeedsAction(state, matrix, row) {
        const definition = matrices.get(matrix);
        if (!definition || matrixStatus(state, matrix, row) !== 'Nee')
            return false;
        const actions = (definition.columns || []).filter((column) => !(definition.statusColumns || []).includes(column));
        return !actions.some((column) => state.matrices?.[core.matrixKey(matrix, row, column)]);
    }
    ...
    FD.MaintenanceFormFlow = {
        ...
        fieldVisible: (name, state) => {
            const field = fields.get(name);
            return Boolean(field && core.fieldIsVisible(field, state.fields || {}, contextFor(state)));
        },
