(function (global) {
    ...
    function createPdfHandlers(ctx) {
        const { elements, controls, modeController, modes, getPdfJsLib, ensureCropperLibrary = async () => global.Cropper, ensureSession, onSave, onSaved, showToast, } = ctx;
        ...
        function showPdfOverview({ discardEditorChanges = true } = {}) {
            ...
            if (ctx.onEditorClosed) {
                Core.hide(elements.pdfEditor);
                ctx.onEditorClosed({ discardEditorChanges });
                return;
            }
        ...
        function mapPreviewCropToRender(cropData, meta, rendered, rotation = 0) {
            const normalizedRotation = ((Number(rotation) % 360) + 360) % 360;
            const swapsAxes = normalizedRotation === 90 || normalizedRotation === 270;
            const previewWidth = Math.max(1, Number(swapsAxes ? meta?.height : meta?.width) || Number(rendered.width) || 1);
            const previewHeight = Math.max(1, Number(swapsAxes ? meta?.width : meta?.height) || Number(rendered.height) || 1);
        ...
        async function highQualityPdfCrop(page, cropData) {
            ...
            if (!pdfDocument || !meta || meta.source !== 'pdf')
                return null;
            const rotation = ((Number(state.pdfEditorRotation || 0) % 360) + 360) % 360;
            ...
            const outputCanvas = FD.PdfImportService.cropCanvas(rendered.canvas, mapPreviewCropToRender(cropData, meta, rendered, rotation));
