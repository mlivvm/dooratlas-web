(function (global) {
    ...
    function setMarkerDoorMetadata(marker, { doorCode, description = '' }) {
    ...
    function clearRuntimeMarkerState(marker) {
        ...
        ['data-fd-marker-key', 'data-fd-api-door-id', 'data-fd-door-code', 'data-fd-door-name'].forEach(attribute => marker.removeAttribute(attribute));
    }
    ...
    function renameChange(doorId, oldCode, newCode, oldDescription = '', newDescription = '') {
        return { type: 'rename', doorId, oldCode, newCode, oldDescription, newDescription };
    }
    ...
    function revertEditChange(change, root, { initMarker } = {}) {
        ...
        if (change.type === 'rename') {
            const marker = findMarkerByDoorId(root, change.doorId || '');
            if (marker)
                setMarkerDoorMetadata(marker, { doorCode: change.oldCode || '', description: change.oldDescription || '' });
            return;
        }
    ...
    function serializeCleanSVG(svgEl, serializer = new XMLSerializer()) {
        ...
        svgClone.querySelectorAll('[data-door-id], [data-fd-marker-key], [data-fd-api-door-id], [data-fd-door-code], [data-fd-door-name]')
    ...
    function markerBox(marker, padding = 0) {
        ...
        const ry = parseFloat(marker.getAttribute('ry') || '') || parseFloat(marker.getAttribute('r') || '') || rx;
        return { left: cx - rx - padding, right: cx + rx + padding, top: cy - ry - padding, bottom: cy + ry + padding };
    }
    ...
    FD.MarkerService = {
        sliderRange, markerRadius, getDoorId, markerDisplayLabel, markerDoorCode, markerDoorDescription, isIgnoredDoorId, prepareInteractiveMarker, setMarkerDoorMetadata, setMarkerRadius,
        ...
        serializeCleanSVG, nextAutoCode, editableBounds, labelBounds, labelPlacements, clampPosition, pointInsideBounds, maxRadiusAtPosition,
    };
})(window);
