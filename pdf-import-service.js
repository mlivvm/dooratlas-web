(function (global) {
    ...
    function pageViewport(page, scale, rotation) {
        // PDF.js applies the PDF's own /Rotate value only when rotation is omitted.
        // A manual edit is a delta on top of that embedded orientation.
        if (rotation === undefined)
            return page.getViewport({ scale });
        const baseRotation = Number(page.rotate || 0);
        const effectiveRotation = ((baseRotation + rotation) % 360 + 360) % 360;
        return page.getViewport({ scale, rotation: effectiveRotation });
    }
    ...
    async function renderPdfPageToCanvas(pdf, pageNumber, { scale = UPLOAD_RENDER_SCALE, rotation, documentRef = document, maxSide = UPLOAD_RENDER_MAX_SIDE, maxPixels = UPLOAD_RENDER_MAX_PIXELS, } = {}) {
        const page = await pdf.getPage(pageNumber);
        const baseViewport = pageViewport(page, 1, rotation);
        ...
        const viewport = pageViewport(page, renderScale, rotation);
    ...
    async function renderPdfPageForEditorPreview(pdf, pageNumber, { rotation, documentRef = document, maxSide = EDITOR_PREVIEW_MAX_SIDE, maxPixels = EDITOR_PREVIEW_MAX_PIXELS, } = {}) {
        const page = await pdf.getPage(pageNumber);
        const baseViewport = pageViewport(page, 1, rotation);
        const scale = editorPreviewScale(baseViewport.width, baseViewport.height, maxSide, maxPixels);
        const viewport = pageViewport(page, scale, rotation);
        const canvas = documentRef.createElement('canvas');
        canvas.width = Math.max(1, Math.ceil(viewport.width));
        canvas.height = Math.max(1, Math.ceil(viewport.height));
        await page.render({ canvasContext: canvas.getContext('2d'), viewport }).promise;
