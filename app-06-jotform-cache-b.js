...
function fetchFloorplanSVGCacheFirst(fileUrl, options = {}) {
...
const pickerCatalogRefresh = FD.PickerCatalogRefreshService.create({
    customerSelect, floorplanSelect,
    getCustomers: () => customers,
    setCustomers: next => { customers = next; },
    loadCustomers: () => FD.DataService.loadCustomers(CONFIG),
    cacheCustomers, populateCustomerDropdown, populateFloorplanDropdown, resetFloorplanDropdown,
    updatePickerButtons,
    isOpen: () => selectionController.isOpen(),
    renderFilters: () => selectionController.renderFilters(),
    renderItems: renderSelectSheetItems,
});
function refreshPickerCatalog() { return pickerCatalogRefresh.refresh(); }
...
function updateAccountIndicator() {
    ...
    const label = displayName || (['admin', 'sales', 'beheer', 'monteur', 'viewer'].includes(role) ? role : '');
...
async function handleExpiredSession(err, context = {}) {
    ...
    try {
        ...
        authController?.showLoginScreen?.({
            message: 'Je sessie is verlopen of je toegang is gewijzigd. Log opnieuw in om verder te gaan.',
            clearPassword: true,
            restoreRemember: true,
        });
        showToast('Sessie verlopen of toegang gewijzigd. Log opnieuw in.', 'error');
        return true;
    }
    finally {
