(function (global) {
    ...
    function createController(options) {
        const { elements, getState, getItems, getFilters, getFilterGroups, getFilterLabel, getFilterValue, getPickerMeta, onFilterChange, onSelect, focusSearchOnOpen = true } = options;
        let activeType = null;
        let activeFilterKey = '';
        ...
        function updatePickerButtons() {
            ...
            if (elements.desktopContextPickerBtn) {
                const showDesktopContext = Boolean(floorplanSelect.value) || document.getElementById('app-container')?.classList.contains('admin-dashboard-active');
                elements.desktopContextPickerBtn.hidden = !showDesktopContext;
                elements.desktopContextPickerBtn.style.display = showDesktopContext ? '' : 'none';
        ...
        function open(type) {
            ...
            setTimeout(() => {
                ...
                else if (!desktopNavigator?.focusSearch(activeType))
                    search.focus();
            }, 0);
        }
        function close() {
