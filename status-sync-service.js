(function (global) {
    ...
    function createController({ sync, intervalMs, getStatus, setStatus, refreshStatus, getState, onStatusChanged, updateDoneButton, showToast, logger = console, }) {
        ...
        async function poll() {
            const { currentFloorplan, isEditMode, online, isVisible } = state();
            if (pollInFlight || !currentFloorplan || isEditMode || online === false || isVisible === false)
                return;
            pollInFlight = true;
            try {
                const refreshed = typeof refreshStatus === 'function'
                    ? await refreshStatus()
                    : await sync.refreshRemoteStatus();
                const result = refreshed && typeof refreshed === 'object' &&
                    Object.prototype.hasOwnProperty.call(refreshed, 'changed') &&
                    Object.prototype.hasOwnProperty.call(refreshed, 'status')
                    ? refreshed
                    : null;
                const nextStatus = result ? result.status : refreshed;
                if (typeof setStatus === 'function')
                    setStatus(nextStatus);
                if (!result || result.changed !== false) {
                    if (typeof onStatusChanged === 'function')
                        onStatusChanged({ source: 'poll', ...(result?.event || {}) });
                    if (typeof updateDoneButton === 'function')
                        updateDoneButton();
                }
                flush();
