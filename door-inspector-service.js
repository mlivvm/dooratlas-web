(function (global) {
    ...
    function createController(input) {
        ...
        function knownFormPresence(door, formType) {
            const value = formType === 'inspection' ? door?.hasOpname : door?.hasMaintenance;
            return typeof value === 'boolean' ? value : null;
        }
        function updateStatusButtons(rows, door = null, rowsLoaded = false) {
            Object.entries(elements.statusButtons || {}).forEach(([formType, button]) => {
                const existing = Boolean(View.latestInspection(rows, formType));
                const known = knownFormPresence(door, formType);
                button?.classList.toggle('has-existing-form', rowsLoaded ? existing : (existing || known === true));
            });
        }
        ...
        function updateActions() {
            ...
            elements.actions.forEach(action => {
                ...
                if (input.fieldappActions) {
                    const existing = View.latestInspection(inspections, formType);
                    action.hidden = !existing;
                    action.textContent = `${TAB_LABELS[formType]} bewerken`;
                    action.disabled = !existing || loading;
                    action.title = loading ? 'Inspectiegegevens laden...' : '';
                    return;
                }
        ...
        function renderHeader(door, rows = inspections) {
            ...
            updateStatusButtons(rows, door, !loading);
            updateActions();
        }
        function renderActiveTab() {
            ...
            elements.body.classList.toggle('door-inspector-body--maintenance-summary', activeTab === 'maintenance');
            if (!inspection) {
        ...
        async function showDoor(doorKey, force = false) {
            ...
            if (!force && door.hasAnyInspection === false) {
                loading = false;
                inspections = [];
        ...
        return {
            bind,
            clear,
            getCachedInspections: (doorId) => cache.has(doorId) ? [...(cache.get(doorId) || [])] : null,
